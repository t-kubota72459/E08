...
- OR 回路、AND 回路も C-MOS (n-MOS と p-MOS を４つ組み合わせる)　で作れる。
　10 分ほど、考えてみよう。
...
OR 回路も同じ要領で NOR 回路を作り (NAND 回路で NOR も作れる。それには **ド・モルガンの公式** が必要だ。そして実際の製品はそのようにできている)、その出力を NOT 回路で否定するように構成する。
...
## EXOR 回路
EXOR の回路を示す。この回路が確かに EXOR を表現しているか、動作を確認してみよう。
